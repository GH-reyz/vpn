...
for user in "${data[@]}"
do
exp=$(grep -w "^### $user" "/etc/xray/vmessgrpc.json" | cut -d ' ' -f 3 | sort | uniq)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
if [[ "$exp2" -le "0" ]]; then
...
for user in "${data[@]}"
do
exp=$(grep -w "^### $user" "/etc/xray/vlessgrpc.json" | cut -d ' ' -f 3 | sort | uniq)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
if [[ "$exp2" -le "0" ]]; then
...
for user in "${data[@]}"
do
exp=$(grep -w "^### $user" "/etc/xray/trojangrpc.json" | cut -d ' ' -f 3 | sort | uniq)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
if [[ "$exp2" -le "0" ]]; then
